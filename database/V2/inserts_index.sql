...
-- Criação de index para kpi de pacientes ativos
CREATE INDEX idx_cp_paciente_id ON consulta_prontuario(paciente_id);
-- Criação de index para a consulta de pacientes e funcionários por area
CREATE INDEX idx_func_ativo_esp ON funcionario(ativo, especialidade);
CREATE INDEX idx_cp_func_paci ON consulta_prontuario(funcionario_id, paciente_id);


SELECT p.*
FROM paciente p
LEFT JOIN consulta_prontuario c ON p.id = c.paciente_id
WHERE c.paciente_id IS NULL;

-- teste indexação
EXPLAIN SELECT p.id, p.nome
FROM paciente p
LEFT JOIN consulta_prontuario c ON p.id = c.paciente_id
WHERE c.paciente_id IS NULL;

SELECT 
    f.especialidade AS Setor,
    COUNT(DISTINCT f.id) AS Total_Funcionarios,
    (SELECT COUNT(DISTINCT cp.paciente_id) 
     FROM consulta_prontuario cp 
     WHERE cp.funcionario_id IN (
         SELECT id FROM funcionario f2 
         WHERE f2.especialidade = f.especialidade AND f2.ativo = 1
     )
    ) AS Total_Pacientes
FROM 
    funcionario f
...
-- testando se a indexação funcionou    
EXPLAIN SELECT f.especialidade, COUNT(f.id) 
FROM funcionario f 
WHERE f.ativo = 1 
GROUP BY f.especialidade;

-- index para o gráfico de pacientes por convenios
CREATE INDEX idx_paciente_convenio ON paciente(convenio);

SELECT 
    convenio AS Seguradora,
...
-- teste index
EXPLAIN SELECT convenio, COUNT(id) 
FROM paciente 
WHERE ativo = 1 
GROUP BY convenio;
...
Select * from consulta_prontuario;
